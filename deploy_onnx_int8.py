"""
YOLOv3 INT8 ONNX Deployment Script
===================================
Loads the quantized yolov3_int8.onnx via ONNX Runtime and runs inference.
Supports image, video, and webcam modes.

Compared to FP32:
  - Model size: 236 MB -> 59 MB (4x smaller)
  - Inference: ~1.4x faster on CPU

Usage:
  python deploy_onnx_int8.py --image path/to/image.jpg
  python deploy_onnx_int8.py --image path/to/image.jpg --save
  python deploy_onnx_int8.py --webcam
  python deploy_onnx_int8.py --video path/to/video.mp4
"""
import argparse
import os
import sys
...
ONNX_PATH = os.path.join(MODEL_DIR, "yolov3_int8.onnx")
...
def load_model():
    if not os.path.exists(ONNX_PATH):
        sys.exit(f"INT8 model not found: {ONNX_PATH}\nRun quantize_int8.py first.")

    print(f"Loading INT8 model from {os.path.basename(ONNX_PATH)}...")
    sess_opts = ort.SessionOptions()
    sess_opts.graph_optimization_level = ort.GraphOptimizationLevel.ORT_ENABLE_ALL
    # Enable more CPU threads for better throughput
...
def postprocess(outputs, frame_h, frame_w, classes):
    ...
    for oi, output in enumerate(outputs):
        ...
        num_anchors = 3
        num_classes = (num_boxes // num_anchors) - 5
        grid_x, grid_y = np.meshgrid(np.arange(gw), np.arange(gh))

        for b in range(num_anchors):
            idx = b * (5 + num_classes)
            tx = 1.0 / (1.0 + np.exp(-out[idx + 0]))
            ty = 1.0 / (1.0 + np.exp(-out[idx + 1]))
            obj = 1.0 / (1.0 + np.exp(-out[idx + 4]))
            cls_scores = 1.0 / (1.0 + np.exp(-out[idx + 5 : idx + 5 + num_classes, :, :]))
            ...
            for y, x in zip(ys, xs):
                ...
                bw = anchor_w * np.exp(float(out[idx + 2][y, x])) / INPUT_SIZE
                bh = anchor_h * np.exp(float(out[idx + 3][y, x])) / INPUT_SIZE
                ...
                bw_px = bw * frame_w
                bh_px = bh * frame_h
                left = int(cx_px - bw_px / 2)
                top = int(cy_px - bh_px / 2)

                boxes.append([left, top, int(bw_px), int(bh_px)])
...
def process_image(image_path, save=False):
    ...
    if save:
        out_path = os.path.splitext(image_path)[0] + "_int8_detected.jpg"
        _, buf = cv2.imencode(".jpg", annotated)
        buf.tofile(out_path)
        print(f"Saved: {out_path}")

    cv2.imshow("YOLOv3 INT8", annotated)
...
def process_webcam():
    ...
    print("Webcam mode (INT8). Press 'q' to quit.")
    frame_count = 0
    t0 = time.time()
    fps = 0.0
    ...
    while True:
        ...
        cv2.putText(annotated, f"FPS: {fps:.1f} (INT8)", (10, 30),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
        cv2.imshow("YOLOv3 INT8 Webcam", annotated)
...
def process_video(video_path):
    ...
    out_path = os.path.splitext(video_path)[0] + "_int8_detected.mp4"
    ...
    while True:
        ...
        if fc % 30 == 0:
            print(f"  frame {fc}")
        cv2.imshow("YOLOv3 INT8 Video", annotated)
        if cv2.waitKey(1) & 0xFF == ord("q"):
            break
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="YOLOv3 INT8 ONNX Deployment")
